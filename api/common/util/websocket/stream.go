			...
			case "resizePty":
				if msgObj.Cols > 0 && msgObj.Rows > 0 {
					...
					if err := r.Terminal.session.WindowChange(msgObj.Rows, msgObj.Cols); err != nil {
						common.LOG.Error(fmt.Sprintf("ssh pty change windows size failed: %v", err))
					}
				}
				return 0, nil
			case "closePty":
				if msgObj.Close {
					...
					if err := r.Terminal.Close(); err != nil {
						common.LOG.Error(fmt.Sprintf("Close pty failed: %v", err))
					}
				}
				return 0, nil
			}
